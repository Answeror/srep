...
def _crossval_predict_aux(self, Mod, get_crossval_val, fold, context, dataset_args=None):
    Mod = deepcopy(Mod)
...
@utils.cached(ignore=['context'])
def _crossval_predict_proba_aux(self, Mod, get_crossval_val, fold, context, dataset_args=None):
    Mod = deepcopy(Mod)
    Mod.update(context=context)
    mod = module.RuntimeModule(**Mod)
    Val = partial(
...
class CrossValEvaluation(Evaluation):
    ...
    def vote_accuracy_curves(self, exps, folds, windows, balance=False):
        acc = []
        for exp in exps:
            for fold in folds:
